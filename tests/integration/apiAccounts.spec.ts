...
test.describe('Actions With Account', () => {
  ...
  test('TC-12077  Back Office - Verify user is able to make a contact inactive', async ({ page, request, loginAPI, accountID, contactID }) => {
    ...
    if (!contactID) {
      throw new Error("contactID is undefined. Please ensure the account is created first.");
    }
  ...
  test('TC-1060 Verify user can add Location to account', async ({ page, request, loginAPI, accountID, contactID }) => {
    const pm = new PageManager(page)
    if (!accountID) {
      throw new Error("accountID is undefined. Please ensure the account is created first.");
    }
    ...
    await pm.onAccountsPageAPI().addLocationToAccount(request, loginAPI, accountID, contactID)
  })
})